...
Bukkit/Minecraft server, run the \'/nolagg\' command, parses the result, and
returns the server\'s TPS (ticks/s) rate for monitoring with Nagios.  This
plugin assumes that \'/nolagg\' is provided by the NoLagg plugin, available
at http://dev.bukkit.org/server-mods/nolagg/', 
...
carefully, and pay particular attention to Table 3: Example Ranges. 
  ...
  Warn when tickrate <19 and critical when tickrate <18:
...
qq{-w, --warning=MIN:MAX
   ...
   warning will be generated.  If omitted, tickrates less than 19
...
qq{-c, --critical=MIN:MAX
   Minimum and maximum tickrate, outside of which a
   critical will be generated.  If omitted, tickrates less than 18
...
if ($rcon->connect){
	my $response = $rcon->command('nolagg');
	my $tps = ($response =~ /Ticks per second: ([\d\.]+)/)[0];
	$rcon->disconnect();
